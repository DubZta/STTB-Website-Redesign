...
public class LocalStorageService : ILocalStorageService
{
    ...
    private string WebRootPath => _webHostEnvironment.WebRootPath ?? Path.Combine(Environment.CurrentDirectory, "wwwroot");
    ...
    public async Task<string> SaveFileAsync(Stream fileStream, string filename, string folderPath, CancellationToken cancellationToken = default)
    {
        ...
        var uploadFolder = Path.Combine(WebRootPath, folderPath.TrimStart('/').Replace('/', '\\'));
