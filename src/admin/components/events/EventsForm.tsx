  ...
  return (
    ...
    >
      <div className="flex-none grid grid-cols-6 gap-x-6 gap-y-4">
        <div className="col-span-2 space-y-1.5">
          ...
          <Input
            ...
            className="h-11 rounded-xl border-slate-200 bg-white text-sm focus:ring-emerald-500/10 focus:border-emerald-500 transition-all shadow-sm"
          />
        </div>
        ...
        <div className="col-span-1 space-y-1.5">
          ...
          <select
            {...register("category")}
            className="w-full h-11 rounded-xl border border-slate-200 bg-white px-4 text-xs font-medium focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all shadow-sm cursor-pointer"
          >
          ...
          <Popover>
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar
                ...
                onSelect={(date: Date | undefined) => setValue('eventDate', date?.toISOString() || '')}
                initialFocus
              />
            </PopoverContent>
          </Popover>
          {errors.eventDate && <p className="text-[9px] text-red-500 font-bold">{errors.eventDate.message}</p>}
        </div>
      ...
      <div className="flex-grow flex flex-col min-h-[300px] space-y-2">
        ...
        <textarea
          {...register("content")}
          className="flex-grow w-full rounded-xl border border-slate-200 p-6 text-sm leading-relaxed resize-none bg-white focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all shadow-sm"
      ...
      <div className="flex-none flex items-center justify-between pt-6 border-t border-slate-100">
        <div className="flex items-center gap-2 text-slate-400">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest">S3 Optimized Storage</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            ...
            ) : isEdit ? "Update Schedule" : "Add to Schedule"}
          </Button>
        </div>
      </div>
    </form>
