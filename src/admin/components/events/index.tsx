...
export default function EventsManagement() {
  ...
  const [viewingItem, setEventDetail] = useState<EventDetail | null>(null);
  ...
  const handleEditClick = async (e: React.MouseEvent, item: EventListItem) => {
    e.stopPropagation();
  ...
  const handleRowClick = async (item: EventListItem) => {
    setLoadingDetail(true);
    const { data } = await fetchGET<EventDetail>(AdminApiUrl.eventsDetail(item.id));
    setLoadingDetail(false);
    if (data) {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm font-medium">
            Coordinate and manage all institutional events and calendars.
          </p>
        </div>
        <div className="flex items-center gap-3 w-full md:w-auto">
          <Button
            variant="outline"
            size="sm"
            className="bg-white rounded-xl h-11 px-6 border-slate-200 text-slate-600 hover:bg-slate-50 font-bold"
          >
      ...
      <AnimatePresence>
        {!!error && (
          ...
          >
            ...
            Unable to sync event data. Authentication might have expired or
              ...
              <TableRow>
                <TableCell colSpan={6} className="py-32 text-center">
                  <div className="flex flex-col items-center gap-4 text-slate-400">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] italic">Syncing Calendar Data...</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : items.length === 0 ? (
          ...
          <DialogHeader className="px-8 py-6 border-b border-slate-100 flex flex-row items-center justify-between bg-white shrink-0">
            <div className="flex items-center gap-4">
              ...
              <DialogTitle className="text-xl font-bold text-slate-900 tracking-tight">
                {editingItem ? "Update Event Registry" : "Register New Event"}
              </DialogTitle>
            </div>
          </DialogHeader>
              ...
              <div className="h-64 w-full relative">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-10">
                  ...
                  <p className="text-white/60 text-xs font-medium mt-2">
                    Event Date: {formatDate(viewingItem.eventDate)} • /
                    {viewingItem.slug}
                  </p>
                </div>
              </div>
              <div className="p-12 prose prose-emerald max-w-none">
                <div className="text-slate-600 leading-relaxed whitespace-pre-wrap font-medium">
                  {viewingItem.content}
                </div>
              </div>
              <div className="px-12 py-8 bg-slate-50 flex justify-end text-right">
                <Button
                  onClick={() => setDetailOpen(false)}
                  className="rounded-xl bg-emerald-600 text-white px-8 font-bold border-none"
                >
                  Close Schedule Preview
                </Button>
              </div>
            </>
          )}
