...
import { Calendar, ArrowLeft, MapPin, Loader2, FileText, User } from 'lucide-react';
...
export default function ActivitiesDetailPage() {
  ...
  const [data, setData] = useState<GetEventDetailResponse | null>(null);
  ...
  const loadData = async (eventId: string) => {
    setLoading(true);
    const { data: responseData } = await fetchGET<GetEventDetailResponse>(AppApiUrl.getEventDetail(eventId));
    if (responseData) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-6">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-gray-500 mb-6 max-w-xs">Kegiatan yang Anda cari tidak tersedia atau telah dipindahkan.</p>
          <button 
            onClick={() => navigate(-1)}
        ...
        <div className="bg-white p-8 lg:p-12 rounded-3xl shadow-2xl border border-white/50 backdrop-blur-xl">
          ...
          <div className="flex flex-wrap items-center gap-8 mb-10 pb-8 border-b border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Tanggal Kegiatan</p>
                <span className="font-bold text-slate-900">{formatDate(data.eventDate, 'id')}</span>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="font-bold text-slate-900">STTB CAMPUS</span>
              </div>
            </div>
            <div className="flex items-center gap-3">
