...
export function Header() {
  ...
  // Handle scroll for sticky header and top bar visibility
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <header className="sticky top-0 z-50 font-sans">
      ...
      >
        {/* Top Bar - Utility Navigation (Dark Blue) */}
      ...
      <div
        className={`bg-white border-b-4 shadow-sm transition-all duration-300 ${
          isScrolled
            ? "border-gray-200 shadow-md" // Subtle when scrolled
            : "border-[#D4AF37] shadow-sm" // Gold at top of page
        }`}
      >
        <div className="container mx-auto px-6 lg:px-8">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {navItems.map((item) => (
                ...
                >
                  <a
                    href={item.href}
                    className={`px-3 py-2 rounded-md text-[13px] font-medium transition-all flex items-center gap-1 ${
                      isActive(item.href)
