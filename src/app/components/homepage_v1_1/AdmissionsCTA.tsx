      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <span className="block text-2xl text-[#D4AF37] font-bold tracking-[0.25em] mb-3">
              VII
            </span>
            <MaskText type="lines">
          ...
          <div className="flex flex-col items-center gap-10">
            <div className="w-full lg:w-3/5 bg-white/10 border border-[#D4AF37]/60 shadow-[0_0_30px_rgba(212,175,55,0.15)] rounded-xl p-10 text-center">
              ...
              <h3 className="text-2xl mb-3 font-bold text-white">{t('admissions.online')}</h3>
              <p className="text-white/75 text-sm mb-6">
                {t('admissions.online_desc')}
              </p>
              <Link to="/admissions">
                <Button
                  size="lg"
                  className="group bg-[#DC2626] text-white hover:!bg-[#B91C1C] border-2 border-[#DC2626] px-8 py-6 text-base uppercase tracking-wider shadow-[0_4px_6px_rgba(220,38,38,0.3)] hover:shadow-lg transition-transform hover:scale-105 font-bold"
                >
                  {t('hero.cta')}
            ...
            <div className="flex flex-col lg:flex-row gap-8 w-full lg:w-4/5">
              <div className="flex-1 bg-white/10 border border-white/20 rounded-lg p-8 text-center transition-all hover:-translate-y-1 hover:bg-white/10 hover:border-[#D4AF37]/30">
                ...
                <h4 className="text-xl mb-2 font-bold text-white">{t('admissions.test')}</h4>
                <p className="text-white/70 text-sm">
                  {t('admissions.test_desc')}
                </p>
              </div>
              ...
              <div className="flex-1 bg-white/10 border border-white/20 rounded-lg p-8 text-center transition-all hover:-translate-y-1 hover:bg-white/10 hover:border-[#D4AF37]/30">
                ...
                <h4 className="text-xl mb-2 font-bold text-white">{t('admissions.announcement')}</h4>
                <p className="text-white/70 text-sm">
                  {t('admissions.announcement_desc')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="text-center mt-12 text-white/70 text-sm">
            <p className="inline-flex items-center gap-2 bg-white/10 px-4 py-2 rounded-full text-white/90 font-semibold">
              ...
              {t('admissions.registration.period')}
            </p>
            <p className="mt-2 text-white">
              {t('admissions.scholarship.info')}
            </p>
            <p className="mt-4 text-sm text-white">
              {t('admissions.alumni.info')}
            </p>
          </div>
        </div>
      </div>
